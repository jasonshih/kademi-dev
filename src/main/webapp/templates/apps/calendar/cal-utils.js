...
function updateEvent(event) {
    log("updateEvent", event.start, event.end);
...
function adjustDates(event, type, deltaDays, minuteDelta) {
    log("adjustStartDate", event.start, event.end);
    $.ajax({
        type: 'POST',
        url: event.url,
        data: {
