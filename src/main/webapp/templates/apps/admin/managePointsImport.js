...
var pointsImportUrl = '/rewards/points/upload';
var importWizardStarted = false;
var importTotalCount = 0;
function initManagePointsImport() {
    initUploads();
}
...
function initUploads() {
    ...
    $('#myWizard').on('actionclicked.fu.wizard', function (evt, data) {
        ...
        if (data.step === 2) {
            ...
            if (selectedCols.indexOf('awardedTo') === -1 || selectedCols.indexOf('points') === -1) {
                // mode is auto and no default group selected and no group field selected
                Msg.error('Please indicate "Awarded To" and "Points" fields in data table');
                importerHead.find('select').first().trigger('focus');
                evt.preventDefault();
                return false;
            }
            
            if (selectedCols.indexOf('rewardName') === -1 && $('#defaultReward').val() === '') {
                Msg.error('Please indicate "Reward name" fields in data table or a default reward');
                evt.preventDefault();
                return false;
            } 
        }
...
function checkProcessStatus() {
    ...
    $.ajax({
        type: 'GET',
        dataType: "json",
        url: pointsImportUrl + "?importStatus",
