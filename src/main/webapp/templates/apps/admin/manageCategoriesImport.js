...
var catImportUrl = '/categories/upload';
var importWizardStarted = false;
var OrgImportTotalCount = 0;
function initManageCategoriesImport() {
    initUploads();
}
...
function initUploads() {
    ...
    $('#myWizard').on('actionclicked.fu.wizard', function (evt, data) {
        ...
        if (data.step === 2) {
            ...
            var selectedCols = [];
            var requiredField = 'categoryCode';
            ...
            if (selectedCols.indexOf(requiredField) === -1) {
                ...
                Msg.error('Please indicate Category code field in data table');
                importerHead.find('select').first().trigger('focus');
                evt.preventDefault();
                return false;
            }
