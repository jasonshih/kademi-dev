...
function initUploads() {
    ...
    $('#myWizard').on('actionclicked.fu.wizard', function (evt, data) {
        ...
        if (data.step === 2) {
            ...
            if (selectedCols.indexOf('amount') === -1 
                    || selectedCols.indexOf('periodFrom') === -1
                    || selectedCols.indexOf('attributedTo') === -1
                    || selectedCols.indexOf('entered') === -1) {
                // mode is auto and no default group selected and no group field selected
                Msg.error('Please indicate "Amount", "Period From", "Attribute To" and "Entered" fields in data table');
                importerHead.find('select').first().trigger('focus');
                evt.preventDefault();
                return false;
            }
