...
function initRsvpForm() {
    ...
    rsvpForm.forms({
        validate: function () {
            ...
            if (option.length === 0) {
                var alert = $("<div class='alert alert-warning'><a class='close' data-dismiss='alert' href='#' aria-hidden='true'>&times;</a> <i class='fa fa-warning'></i> Please select an option</div>");
            ...
            if (response.fieldMessages.length > 0 && response.fieldMessages[0].field === "userData") {
                ...
                jQuery.ajax({
                    ...
                    success: function (resp) {
                        ...
                        flog(resp);
                        var page = $(resp);
                        var form = page.find("#form-details");
                        form.find('.grid-title').remove();
                        form.find('.grid-body').attr('class', '');
                        form.find("p.alert").html("<i class='fa fa-info-circle'></i> In order to register for this event you must provide the following information.");
                        form.find("#firstName").addClass("required");
                        form.find("#surName").addClass("required");
                        form.find(".form-action").remove();
