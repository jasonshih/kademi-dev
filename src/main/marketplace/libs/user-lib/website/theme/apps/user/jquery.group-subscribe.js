...
(function($) {
    ...
    var methods = {
        init: function(options) {
            ...
            flog("checkbox?", config.isCheckbox, "button?", config.isButton);
            // set initial state
            initSubscribeInitialState(container, config);

            flog("init group subscribe", container);
            ...
            if (config.subscribeEvents !== null) {
                flog("listen for subcribe events", config.subscribeEvents);
    ...
    try {
        ...
        $.ajax({
            ...
            success: function(resp) {
                ajaxLoadingOff();
                container.removeClass("ajax-loading");
                flog("changeSubscribeState: finished", newSelectedVal, container);
