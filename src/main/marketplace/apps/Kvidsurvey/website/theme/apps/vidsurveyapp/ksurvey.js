...
function initCheckboxes() {
    $('.answers-vidsurvey input[type=checkbox]').on('click', function (e) {
...
function initGotoCurrentQuestion() {
    var questions = $('#vidsurvey-questions').find('.question-item[data-answered=false]');
    if (questions.length > 0) {
        questions.addClass('hide');
        var questionId = questions.first().attr('data-questionId');
        var index = questions.first().removeClass('hide').index();
        $('#vidsurvey-questions').find('.question-index').text(index+1);
    }

}

function checkSubmitButton() {
    var done = $('#vidsurvey-questions').find('.question-item[data-answered=false]').length < 1;
    if (done) {
        $('#btn-submit-vidsurvey').removeClass('hide');
        $('.vidsurvey-submit-alert').removeClass('hide');
        $('#vidsurvey-questions').find('#vidsurvey-question-navigation').addClass('hide');
    }
}
...
function initVideoRecorder(maxLength) {
    ...
    playerRecorder = videojs("myVideo-vidsurvey", opt);
        ...
        if (playerRecorder && playerRecorder.recordedData) {
            ...
            $.ajax({
                ...
                if (resp && resp.status && resp.data) {
                    $('#vidsurvey-questions').reloadFragment({
...
function initCountdown() {
    var countdownT = setInterval(function(){
        if(!$('#vidsurvey-cowndowntime').length){
            clearInterval(countdownT);
            return;
        }
        var startTime = $('#cowndowntime').attr('data-startTime');
        $('#vidsurvey-cowndowntime').text(countdown(new Date(startTime)).toString());
    },1000)
}
