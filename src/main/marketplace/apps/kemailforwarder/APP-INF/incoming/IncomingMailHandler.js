    ...
    g._storeMail = function (page, toAddr, msg) {
        ...
        if (isNotNull(user)) {
            ...
            securityManager.runAsUser(user, function () {
                ...
                log.info("incomingEmail: {}", incomingEmail);
                var json = JSON.stringify(incomingEmail);
                log.info("EmailForwarder: Saving JSON={}", json);
                db.createNew(g._config.RECORD_NAMES.RECEIVED_EMAIL(toAddress, page.website.name), json, g._config.RECORD_TYPES.RECEIVED_EMAIL);
            });
        }
    };
