...
function doLogin(userName, password, config, container) {
    log("doLogin-bootstrap", userName, config.urlSuffix);
    ...
    $.ajax({
        ...
        error: function(resp) {
            ...
            log("error response from server, set message. msg output:", val, "config msg:", config.loginFailedMessage, "resp:", resp);
            if( val.length === 0 ) {
