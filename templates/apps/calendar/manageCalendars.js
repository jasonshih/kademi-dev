...
function initManageEvent() {
    ...
    reminderForm.submit(function(e) {
        ...
        if( timerUnit === "" ) {
            alert("Please select the time units, like how many days, hours, etc");
            return;
        }
        
        if( timerMultiple === "" ) {
            alert("Please select the number of " + timerUnit );
            return;
        }        
        
        if( subject === "" ) {
            alert("Please enter a subject for the reminder email");
            return;
        }           
        
        if( html.length < 5 ) {
            alert("Please check your email message, it looks a bit short");
            return;
        }            
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data.status) {
                ...
                alert("Failed to create the group email " + data.mssages);
            }
        },
        error: function(resp) {
            log("error", resp);
            alert("Could not create the group email. Please check your internet connection, and that you have permissions");
        }
